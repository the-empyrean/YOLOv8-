...
class Ui_MainWindow(QMainWindow):
    ...
    def displayFrames(self):
        ...
        if frames:
            # 使用 YOLO 模型检测物体
            model = YOLO('runs/detect/train4_100e/weights/best.pt')
            results = model(frames)  # 一次性处理整个批次
            ...
            for i, result in enumerate(results):
                ...
                for name, count in self.counts.items():
                    if count > 0:
                        self.targetInfoBrowser.append(f"检测到的物体：{name}，总数量：{count}\n")
    ...
    def startProgram(self):
        ...
        for name, count in counts.items():
            ...
            if count == 0:
                continue
            self.targetInfoBrowser.append(f"检测到的物体：{name}，数量：{count}\n")
